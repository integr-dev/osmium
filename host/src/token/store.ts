  ...
  private async mutate<T>(change: (entries: Entry[]) => T): Promise<T> {
    const run = this.queue.then(async () => {
      ...
      try {
        ...
        await persist(entries, this.path)
...
/**
 * Writes the store out, readable only by its owner.
 *
 ...
 */
async function persist(entries: Entry[], path: string): Promise<void> {
...
/**
 * An exclusive lock on the store, held across a read-modify-write and no longer.
 *
