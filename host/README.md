...
| `path.mode` | `fly` to fly rather than walk, wherever flying is possible. Unset walks. A flying agent flies straight where the hull clears the line, up over and down onto a destination close by where it does not - only as high as the air along the way needs, not as high as the tallest thing above it - and searches through the air only where neither works; a long flight climbs twelve blocks clear of the ground under the stretch ahead, higher over whatever is in the way - up to the build limit - before turning round it or searching, and stays at that height until it glides down onto the destination. Every change of height is flown along the way on the shortest route the air allows: the longest clear slope, a descent that starts as soon as nothing is in its way, and coming part of the way down first where a cliff or an overhang is in the way of every slope from higher up - never straight up or down where a slope is clear. Flying is possible where the server grants it - the `abilities` packet's may-fly flag, which creative sets - or where `path.forceFly` says to fly anyway; anywhere else the agent walks and raises a `system`/`warning` activity entry saying why, and a flight the air has no way through is walked from where it stopped. Water is flown through as freely as air - a player with flight is not slowed by it - though air is preferred and a water surface counts as ground when choosing how high to fly. The walking settings above do not apply to a flight. Read when a waypoint is sought. |
