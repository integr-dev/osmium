 ...
 * a key as a path separator: `t('permission.chat.read')` looks for `permission → fleet →
 * chat → read` and silently falls back, since the key is the flat string `'chat.read'`. Node
 * ids contain dots by
